...
- **Production**: `git push upstream master` → Vercel deploys automatically. No manual step.
- **Staging**: branch `staging` is protected on GitHub (PR-only writes from upstream). Two ways to ship:

   **Standard (PR-based, recommended)** — work on a feature branch off `master`, open a PR targeting `staging`, merge:
   ```bash
   git checkout master && git pull upstream master
   git checkout -b feat/<short-name>
   # ... commits ...
   git push upstream feat/<short-name>
   gh pr create --base staging --head feat/<short-name>
   # merge PR → Vercel auto-deploys to dev.run.hopsworks.ai
   ```

   **Hot-iteration (CLI bypass)** — when you want to flash-test changes without going through a PR:
   ```bash
   git checkout staging
   git reset --hard upstream/staging   # or merge in the branch you want to test
   vercel deploy --target=staging --yes
   ```
   Uploads the working tree directly to Vercel and assigns it to the `staging` Custom Environment. Doesn't update the GitHub branch — next PR merge will overwrite whatever you flashed.

   Both paths land on https://dev.run.hopsworks.ai. Full setup: `docs/DEPLOY_STAGING.md`.
- **Preview (PR)**: every PR against master gets a Vercel preview URL.
